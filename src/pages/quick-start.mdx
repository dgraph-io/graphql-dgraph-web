...
It's a one-liner to bring up Dgraph with GraphQL.  *Note: The Dgraph standalone image is great for quick start and exploring, but it's not meant for production use.  Once you want to build an App or persist your data for restarts, you'll need to review the [admin docs]({{< relref "admin/index.md" >}}).*
...
Depending on if you need a bit more of a walkthrough or if you're off and running, you should check out out the worked example or the sample React app.
