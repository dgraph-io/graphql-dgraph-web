...
Starting Dgraph with GraphQL can be done by running from the all-in-one docker image.  *Note: The Dgraph standalone image is great for quick start and exploring, but it's not meant for production use.  Once you want to build an App or persist your data for restarts, you'll need to review the  [admin docs](/doc/admin).*
