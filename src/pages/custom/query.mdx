---
title: 'Custom Queries'
---

<h1>{props.pageContext.frontmatter.title}</h1>

Let's say we want to integrate our app with an existing external REST API.  There's a few things we need to know:

* the URL of the API, the path and any parameters required
* the shape of the resulting JSON data
* the method (GET, POST, etc.), and
* what authorization we need to pass to the external endpoint

The custom query can take any number of scalar arguments and use those to construct the path, parameters and body (we'll see an example of that in the custom mutation section) of the request that gets sent to the remote endpoint.

Here's an example calling the Twitter API

```
type TwitterUser @remote {
    id: ID!
    name: String
    screen_name: String
    location: String
    description: String
    followers_count: Int
    ...
}

type Query{
    getTwitterUser(screen_name: String!): TwitterUser @custom(http:{
        url: "https://api.twitter.com/1.1/users/show.json?screen_name=$screen_name"
        method: "GET",
        forwardHeaders: ["Authorization"]
    })
}
```

Dgraph takes a GraphQL query like

```
query {
    getTwitterUser(screen_name: "dgraphlabs") {
        location
        description
        followers_count
    }
}
```
 
constructs a HTTP GET request to `https://api.twitter.com/1.1/users/show.json?screen_name=dgraphlabs` attaches header `Authorization` from the incoming GraphQL request to the outgoing HTTP, and makes the call.

The result JSON will contain the whole object from the REST endpoint (you can see how much is in the Twitter user object [here](https://developer.twitter.com/en/docs/tweets/data-dictionary/overview/user-object)).  But the GraphQL query only asked for some of that, so Dgraph filters out any returned values that weren't asked for in the GraphQL query and builds a valid GraphQL response to the query and returns GraphQL.

```
{
    "data": {
        "getTwitterUser": { "location": ..., "description": ..., "followers_count": ... }
    }
}
```

Your version of the remote type doesn't have to be equal to the remote type.  For example, if we don't want to allow users to query the full Twitter user, we can include in the type definition only the fields we want to allow.

All the usual options for custom queries are allowed; for example, you can have multiple queries in a single GraphQL request and a mix of custom and Dgraph generated queries, you can get the result compressed by setting `Accept-Encoding` to `gzip`, etc.

---
