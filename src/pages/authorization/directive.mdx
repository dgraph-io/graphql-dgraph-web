...
Given an authentication mechanism and signed JWT, it's the `@auth` directive that tells Dgraph how to apply authorization.  The directive can be used on any type (that isn't an `@remote` type) and specifies the authorization for `query` as well as `add`, `update` and `delete` mutations.
...
The rule expects the JWT to contain a claim `"USER": "..."` giving the username of the logged in user, and says: you can query any todo that has your username as the owner.
...
Often you authorization depends not on the object being queried, but on the connections in the graph that object has or doesn't have.  Because the auth rules are graph queries, you can express very powerful graph search and traversal as the auth rules.
...
so your auth rule for todos depends not on a value in the todo, but on checking which author it's linked to.  This means our auth rule must make a step further into the graph to check who the author is.

```graphql
query ($USER: String!) { 
    queryTodo {
        author(filter: { username: { eq: $USER } } ) { 
            username
        } 
    } 
}
...
You can also express rules that can be administered by the app itself.  You might define type `Role` and enum `Privileges` that can have values like `VIEW`, `ADD`, etc. and state in your auth rules that a user needs to have a role with particular privileges to query/add/update/delete and those roles can then be allocated inside the app.  For example, stating that when a project is created the admin can decide which users have view or edit permission, etc.

# Role Based Access Control

As well a rules that relate a user's claims to a graph traversal, role based access control rules are also possible.  These rules relate a claim in the JWT to a known value.  

For example, perhaps if someone logs in as the `ADMIN`, they are the only ones allowed to delete users.  For that, we might expect the JWT to contain a claim `"ROLE": "ADMIN"`, and can thus express a rule that only allows users with the `ADMIN` claim to delete.
...
Not all claims need to be present in all JWTs.  For example, if the `ROLE` claim isn't present in a JWT, any rule that relies on `ROLE` simply evaluates to false.  As well as simplifying your JWTs (e.g. not all users need a role if it doesn't make sense to do so), this means you can also simply disallow some queries and mutations.  If you know that your JWTs never contain the claim `DENIED`, then
...
can never be true and this prevents users ever being deleted.
...
For example, in Twitter, StackOverflow, etc. you can see authors and posts without being signed it - but you'd need to be signed in to add a post.  With Dgraph auth rules, if a type doesn't have a `query` auth rule or the auth rule doesn't depend on a JWT value, then the data can be accessed without a signed JWT.

For example, the todo app might allow you to view any author, but not make any mutations unless you are the author or an admin.  That would be achieved by this kind of setup.
...
Maybe some todos can be marked as public and anyone can see those.
